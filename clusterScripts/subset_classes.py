...
def pick_balanced_subset_classes(data:list, classes:list, class_size:int, seed = 1998, do_shuffle = True) -> list:
    """
    ...
    for img in tqdm(range(0, data[1].shape[0])):
        label = data[1][img]
        #print("label",label)
        for num in classes:
            #print("num",num)
...
"""
#Creating the balanced train and validation set of 7 classes
...
full_npz_train_path = "/mnt/server-home/TUE/20175985/BepDataResNet/npzData/trainDataNotProcessedFull.npz"
train_df = model.read_train_data(path = full_npz_train_path)
out_train = pick_balanced_subset_classes(train_df, [0,1,2,3,4,5,6], 150_000)
np.savez_compressed(
    "/mnt/server-home/TUE/20175985/BepDataResNet/npzData/trainDataNotProcessedFull7Classes", X_train = out_train[0],
     Y_train = out_train[1])
